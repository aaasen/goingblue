...
# Supported codec services. Each admits only the runtime account as an invoker (deploy-codec.sh),
# so the gateway sends an identity token with every codec call (CODEC_AUTH=iam, identity.ts).
