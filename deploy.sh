...
gcloud run deploy "$SERVICE" --project "$PROJECT" --source . --region "$REGION" \
  ...
  --set-env-vars "GOOGLE_CLOUD_PROJECT=$PROJECT,INSTANCE_CONNECTION_NAME=$INSTANCE_CONNECTION_NAME,DB_USER=$DB_USER,DB_NAME=$DB_NAME,TWILIO_WEBHOOK_URL=$TWILIO_WEBHOOK_URL,CODEC_URL_V1=$CODEC_URL_V1,CODEC_URL_V2=$CODEC_URL_V2,CODEC_URL_V3=$CODEC_URL_V3,CODEC_URL_V4=$CODEC_URL_V4" \
