...
// time (24h → 12h → 6h → 3h → 1h). The wire carries only a sequence number `seq` — both sides
...
//
// Periods align to LOCAL midnight (a 24h period starts at 0:00; 12h at 0:00/12:00; 6h at
...
//
// The sequence:
//   seq = 1..D-1   truncated: seq whole days, all at 24h (fallback when even 24h × D
//                  doesn't fit the budget)
//   seq = D        the full duration, all at 24h
//   seq = D + j    (j = 1..D)  the first j days at 12h, the rest at 24h
//   seq = 2D + j   (j = 1..D)  the first j days at 6h,  the rest at 12h
//   seq = 3D + j   (j = 1..D)  the first j days at 3h,  the rest at 6h
//   seq = 4D + j   (j = 1..D)  the first j days at 1h,  the rest at 3h
// so seq = 5D is the whole window at 1h. Stage boundaries coincide (seq = kD as "j = D of stage
...
// The refinement ladder is exactly the resolution indices 0..4 (see RESOLUTION_HOURS).
export const FILL_STAGES = 5;

export interface FillLayout {
  seq: number;
  durationDays: number;
  // seq < durationDays: pure-24h forecast covering fewer days than requested.
  truncated: boolean;
...
export function layoutFor(
  ...
  // Refinement progress t = 0..4D past the all-24h layout: the first nFine days sit at
  // resolution index `fine`, the rest one step coarser. t = 0 (and every truncated layout)
  // is uniform 24h, expressed as "all days at stage 0".
  const t = truncated ? 0 : seq - durationDays;
  const fine = t === 0 ? 0 : Math.ceil(t / durationDays);
  const nFine = t === 0 ? days : t - (fine - 1) * durationDays;
