...
describe.each(CLOUD_BAND_LEVELS_HPA.map((hpa, li) => ({ hpa, li })))(
  "cloud band $hpa hPa entropy coding", ({ li }) => {
    // Cost of a value column with the context threaded the way wire.ts threads it: each step's
...
// Cost of a delta sequence with the context threaded the way wire.ts threads it: each delta's
...
describe("temperature delta entropy coding", () => {
  ...
  it("throws on deltas outside the escape field's range instead of silently truncating", () => {
    ...
    // wire.ts clamps before calling (see the healing round-trip test in encoding.test.ts).
