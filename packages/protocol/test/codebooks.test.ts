...
const FROZEN_DIGESTS: Record<number, string> = {
  1: "6a231d85c8b26b8c", // SHIPPED — v1 has deployed clients; changing this breaks phones in the field
};
...
describe("codebook wire-format freeze", () => {
  it("has a frozen digest recorded for the current protocol version", () => {
    expect(FROZEN_DIGESTS[V1_VERSION],
      `no frozen codebook digest for protocol v${V1_VERSION} — record "${digest}" in FROZEN_DIGESTS`,
    ).toBeDefined();
  });

  it(`v${V1_VERSION} codebooks match the digest frozen when the version shipped`, () => {
    expect(digest,
      "codebooks changed without a protocol version bump — see the comment on FROZEN_DIGESTS",
    ).toBe(FROZEN_DIGESTS[V1_VERSION]);
  });
});
