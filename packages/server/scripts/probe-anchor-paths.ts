 ...
 * aggregation + codec (buildLayoutMessage → v1EncodeBreakdown). Per step it reports fit % against
 ...
 * final anchor table.
 *
 *   node packages/server/scripts/probe-anchor-paths.ts                 # eval split, 160c
 *   node packages/server/scripts/probe-anchor-paths.ts --limit 100     # quick pass
 *   node packages/server/scripts/probe-anchor-paths.ts --split all --max-chars 160
 *
...
import {
  RESOLUTION_HOURS, VARS_BIT, v1EncodeBreakdown,
...
const MAX_PERIODS = 256; // V1_MAX_PERIODS — every candidate layout must stay under it
...
function main(): void {
  ...
  for (const cell of limited) {
    ...
    const bdFor = (p: Profile, comboId: string, mask: number) => {
      ...
      if (!bdMemo.has(k)) {
        const bd = v1EncodeBreakdown({ ...msgFor(p)!, vars_mask: mask });
