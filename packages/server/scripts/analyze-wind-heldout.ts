...
import { quantizeFreqs, RANS_PROB_BITS, V1_MAX_PERIODS, VARS_BIT, type Period } from "@weather/protocol";
...
await eachForecast((hourly: HourlyData, runHour: number, loc: string) => {
  ...
  for (const resIdx of RES_INDICES) {
    ...
    const n = Math.min(V1_MAX_PERIODS, Math.floor(hourly.time.length / hpp));
    if (n < 2) continue;
