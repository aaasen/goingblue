 ...
 * 2026-07-31, was surface → gust): gust decodes FIRST (WIND_COLUMNS order in v2.ts) and lends
 ...
 * must match v2.ts): deltas -17..17 (35 symbols), no escape needed. The surface fallback for
 ...
 * same quantizer as v2.ts. The 24h row (resolution index 0) is trained even though fill layouts
...
const FORCE_MAX = 17;              // extended Beaufort domain, must match v2.ts
