 ...
 * aggregation + codec (buildLayoutMessage → v2EncodeBreakdown). Per step it reports fit % against
...
import {
  RESOLUTION_HOURS, VARS_BIT, v2EncodeBreakdown, V2_VERSION,
...
const MAX_PERIODS = 256; // V2_MAX_PERIODS — every candidate layout must stay under it
...
function main(): void {
  ...
  for (const cell of limited) {
    ...
    const bdFor = (p: Profile, comboId: string, mask: number) => {
      ...
      if (!bdMemo.has(k)) {
        const bd = v2EncodeBreakdown({ ...msgFor(p)!, vars_mask: mask });
