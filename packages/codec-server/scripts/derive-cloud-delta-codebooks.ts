 ...
 * PREVIOUS VALUE): the quantized cover step (0..7, 3-bit — see the cloud band column in v4.ts)
 ...
 * resolution (cloudBandPeriodCount / CLOUD_BAND_MAX_HOURS in v4.ts), so 3h and 1h are the only
 ...
 *
 * WHAT IS COUNTED. The v4 wire carries coverage at each CLOUD_BAND_LEVELS_HPA level, and what
 ...
 * IS the previous quantized step, so v4.ts indexes the books with no mapping in between.
 *
...
const STEP_BITS = 3;               // matches the cloud band column width in v4.ts (steps 0..7)
...
const CLOUD_MASK = 1 << VARS_BIT.cch; // v4: the whole band rides this one bit
