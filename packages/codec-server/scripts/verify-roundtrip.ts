 ...
 * production path (encodeFillSeq: layout windows → aggregation → toFullPeriod → the v3 codec)
...
import {
  v3MessageToString, v3MessageFromString, layoutFor, maxFillSeq, DEFAULT_VARS_MASK,
  ALWAYS_VARS_MASK, CODECS, V3_VERSION,
    ...
    for (const mask of [ALL_VARS, DEFAULT_VARS_MASK, ALWAYS_VARS_MASK]) {
      for (const mode of [MODE_DETAIL, MODE_AUTO, MODE_RANGE]) {
        const params: ForecastParams = {
          ...
          maxChars: 160, decoderVersion: V3_VERSION, code: messages % 128,
        ...
        for (let seq = 1; seq <= maxFillSeq(mode); seq++) {
          ...
          try {
            ...
            const decoded = v3MessageFromString(encoded, () => ctx); // assertDone throws on desync
            ...
            const reencoded = v3MessageToString(decoded);
