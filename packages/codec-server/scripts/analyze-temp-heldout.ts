...
import { V4_CODEBOOKS, RANS_PROB_BITS } from "@weather/protocol";
...
// Only runs while V4_CODEBOOKS still carries flat selector tables; after the A4r adoption the
...
const shippedFlat = (V4_CODEBOOKS.tempDelta as { weights?: number[][] }).weights;
if (shippedFlat) {
  const bits: Record<number, number> = Object.fromEntries(RES_IDXS.map((r) => [r, 0]));
  const n: Record<number, number> = Object.fromEntries(RES_IDXS.map((r) => [r, 0]));
  for (const c of chains) {
