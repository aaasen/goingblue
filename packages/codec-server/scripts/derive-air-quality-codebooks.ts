...
// catches (WIRE_CODEBOOKS carries the mode sets alongside the weights).
const RESIDUALS: {
...
export function counter(): CellCounter {
  const tables = [
    ...
    // them the wire actually uses is wire.ts's call (AQI_*_RESIDUAL_MASKS).
  ...
  return {
    ...
    countCell(ctx, add) {
      ...
      for (let res = 0; res < NRES; res++) {
        ...
        // AQ_HORIZON_HOURS (aqPeriodCount in wire.ts), so periods past it are never emitted — and
