...
| `/twilio-sink` 403, `sink.invalid_signature` | The event's signature check failed: a sink URL mismatch, or a delivery that is not Twilio's | `TWILIO_SINK_URL` against the sink's destination in the Twilio console. Twilio retries the event, so the line repeats until fixed |
